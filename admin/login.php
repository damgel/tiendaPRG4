...
if (isset($_POST['action'])) {
    ...
    if (isset($username) && !empty($password)) {
        $query = mysql_query("SELECT idadmin, nombre FROM usuario WHERE usuario = '$username' AND password = '$password' LIMIT 1");
        while ($row = mysql_fetch_array($query)) {
            session_start();
            $_SESSION['userida'] = $row{'idadmin'};
            $_SESSION['usernamea'] = $row{'nombre'};
            header("Location: administrarProductos.php"); /* Si el usuario existe, direccionar a la pagina princial( catalogo) */
        }
    } else {
...
<html lang="es-ES" class="no-js"> 
    ...
    <head>
        ...
        <style>
            ...
            #frm-login
            {
                display: block;    /*tambien puede poner float:right, para que se alineé a la derecha */
                padding:10px;
                width:60%;
                margin:auto; 
